...
| `ADMIN_EMAIL` / `ADMIN_PASSWORD` | 관리자 계정. 회원가입 없이 이 값으로만 들어갑니다. 교사 로그인 화면에서 이 이메일로 로그인하면 관리자 화면이 열립니다. |
...
0. **관리자** — 교사 로그인 화면에서 `ADMIN_EMAIL` / `ADMIN_PASSWORD`로 로그인하면 관리자 화면이 열립니다. 회원가입은 하지 않습니다. 여기서 자율·진로 활동의 날짜와 응답 양식을 만듭니다. 활동은 학교 전체 공통입니다.
