...
import { runWithReadMeter } from "./firebase/read-meter";

/**
 * 모든 API 라우트의 공통 래퍼.
 * - ApiError는 상태코드와 한국어 메시지를 그대로 내려준다.
 * - 그 외 예외는 500으로 감싸되, 서버 로그에는 원문을 남겨 진단이 가능하게 한다.
 * - 이 요청이 Firestore 를 몇 번 읽었는지 x-fs-reads 헤더로 돌려준다. (큰 읽기 기준)
 */
export function route<T>(handler: () => Promise<T>) {
  return runWithReadMeter((meter) => {
    // 핸들러가 끝난 뒤 meter.n 을 읽어 헤더에 넣는다. (핸들러 도중 addReads 로 누적된다)
    const withHeader = (res: NextResponse) => {
      res.headers.set("x-fs-reads", String(meter.n));
      return res;
    };
    return handler().then(
    (data) => withHeader(NextResponse.json(data ?? { ok: true })),
    (err: unknown) => {
